...
type ResourceInputMethod interface {
	...
	ProcessResource(ctx context.Context, resource *Resource, credentials map[string]string) (result *ResourceInputMethodResult, err error)
}
...
type ResourceConsumerIdentityProvider interface {
	GetResourceCredentialConsumerIdentity(ctx context.Context, resource *Resource) (identity runtime.Identity, err error)
}

// SourceConsumerIdentityProvider resolves the identity of the given [Source] to use for credential resolution.
type SourceConsumerIdentityProvider interface {
	GetSourceCredentialConsumerIdentity(ctx context.Context, source *Source) (identity runtime.Identity, err error)
}
